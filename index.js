    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <div>
          ...
          <div>
            <p>Total Weekdays Events: <span id="countWeekDayEvent">0</span> </p>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    eventForm.addEventListener('submit', function (e) {
      ...
      if (selectedDay === "Saturday" || selectedDay === "Sunday") {
        WeekEndsTaskList.push(`${selectedDay}: ${inputEvent}`)
        count2++;
        updateWeekEndsTask();
      } else {
        WeekDayTaskList.push(`${selectedDay}: ${inputEvent}`)
        count1++;
        updateWeekDaysTask();
      }
    });
    ...
    function updateWeekEndsTask() {
      ...
      for (let i = 0; i < WeekEndsTaskList.length; i++) {
        storage += `<div class="border p-2 px-3"><div class="d-flex justify-content-between"><p class="card-text m-0 pt-1">${WeekEndsTaskList[i]}</p><button onClick="deleteEventWeekEnds(${i})" class="btn btn-danger">Delete</button></div></div>`;
      }
    ...
    function updateWeekDaysTask() {
      ...
      for (let i = 0; i < WeekDayTaskList.length; i++) {
        storage2 += `<div class="border p-2 px-3"><div class="d-flex justify-content-between"><p class="card-text m-0 pt-1">${WeekDayTaskList[i]}</p><button onClick="deleteWeekDayEvent(${i})" class="btn btn-danger">Delete</button></div></div>`;
      }
